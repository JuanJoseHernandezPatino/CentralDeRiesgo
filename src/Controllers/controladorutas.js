  ...
  try {
    if (req.session.user) {
      if (req.session.user.lastVisit + (600 * 1000) > Date.now()) {
        ...
        let tituloP = " Al tablero del control"
          ...
          if (Extension.toUpperCase() == "XLSX" || Extension.toUpperCase() == "XLS") {
            ...
            if (result) {
              datax.forEach(async (row) => {
                ...
                const pool1 = await conex.getConnection();
                const result = await pool1.request().query
                  (`select * from Afiliados where Identificacion=${row.Identificacion.toString()}`)
                ...
                if (tam > 0) {
                  if (basedb.eps != row.eps) {
                    ...
                    await pool
                      .request()
                      .input('Identificacion', sql.VarChar, row.Identificacion.toString())
            ...
            let error_ = ["no valido"];
            return res.send(error_)
          }
        }
        catch {
          let error_ = ["sin archivo"];
          return res.send(error_)
        }
      }
      else {
...
async function datosAfiliados(req, res) {
  try {
    // let idUs = req.body.identificacionUsu;
    //traer lista 
    const pool2 = await conex.getConnection();
    const result = await pool2.request().query(`select * from Afiliados`)
    ...
    if (req.session.user) {
      if (req.session.user.lastVisit + (600 * 1000) > Date.now()) {
        ...
        try {
          ...
          //traer listado
          const pool1 = await conex.getConnection();
          const resultx = await pool1.request().query(`select * from Gestion where concat(identificacionUsu,' ', nombre ,' ', eps) like '%${idUsuario}%'`)
...
async function grafico1(req, res) {
  try {
    if (req.session.user) {
      if (req.session.user.lastVisit + (600 * 1000) > Date.now()) {
        req.session.user.lastVisit = Date.now();
        ...
        collection.aggregate([
          { $group: { _id: "$hFechaGestion", count: { $sum: 1 } } },
          { $sort: { _id: -1 } },
          { $limit: 10 }
...
//para grafico traer las cohortes
async function selecioncohorte(req, res) {
  try {

    let cohorte = req.body.cohorte;
    if (req.session.user) {
      if (req.session.user.lastVisit + (600 * 1000) > Date.now()) {
        req.session.user.lastVisit = Date.now();
        const collection = m.db.collection('Usuarios');
        collection.aggregate([
          { $match: { hCodigo: cohorte } },
          { $group: { _id: "$hFechaGestion", count: { $sum: 1 } } },
          { $sort: { _id: -1 } },
          { $limit: 10 }
        ]).toArray(function (err, resultados) {
...
//mostrar las cohortes a la página
async function cargar(req, res) {
  try {
    const pool4 = await conex.getConnection();
    const resultc = await pool4.request().query(`select * from cohorte where visible= 'si'`)
    const listacohortes = resultc.recordset
    return res.send({ listacohortes });
...
async function graficoCohortes(req, res) {
  try {
    const collection = m.db.collection('Usuarios');
    collection.aggregate([
      { $group: { _id: "$hCohorte", count: { $sum: 1 } } },
      { $sort: { _id: -1 } },
      { $limit: 10 }
    ]).toArray(function (err, resultados) {
      if (err) {
...
module.exports = {
  ...
  datosF: datosF,
  grafico1: grafico1,
  selecioncohorte: selecioncohorte,
  cargar: cargar,
  datosAfiliados: datosAfiliados,
  graficoCohortes: graficoCohortes
}